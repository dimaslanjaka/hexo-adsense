...
module.exports = function modExports(args) {
  ...
  if (!args._.join('')) {
    console.log(
      chalk.red(
        'You need to search for a specific post before you can remove it. Check `hexo help remove` for usage details.'
      )
    );
    process.exit();
  } else {
  ...
  this.load().then(
    function loadDb() {
      ...
      getArticles(locals)
        .then(function loadArticles(articles) {
          ...
          .then(function getStats(stats) {
            if (stats.isFile()) {
              ...
              fsUnlink(src).then(function unlinkFile() {
                ...
                fsStat(assetDir)
                  .then(function getAssetDirStats(adStats) {
