    ...
    function loadDB() {
        ...
        return fsStat(dir)
          .then(function doCheckDir(stats) {
            if (stats.isFile()) {
              console.log(
                chalk.gray(dir),
                'seems to be a file on your filesystem! It needs to be either non-existent or a directory, so please rename your _exile post and run',
                chalk.yellow('hexo isolate'),
          ...
          .catch(function catchCheckDir() {
            ...
            console.log(
              chalk.red('No _exile dir detected. Have you run'),
              chalk.yellow('hexo isolate'),
              chalk.red('first?')
            );
            return false;
      ...
      function getExiledPosts(dir) {
        return fsReaddir(dir)
          .then(function doReadDir(contents) {
            return contents;
          })
