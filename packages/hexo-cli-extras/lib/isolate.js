...
module.exports = function modExports(args) {
  ...
  this.load().then(
    function loadDB() {
      ...
      isDir(exileDir)
        .then(function checkExileDir(exists) {
          if (!exists) {
            makeDir(exileDir);
          }
        })
        ...
        .then(function allDone(isolated) {
          console.log(
            chalk.gray(isolated),
            'successfully isolated.\nTo restore the exiled posts, run',
            chalk.yellow('hexo integrate')
          );
        })
        ...
        return fsStat(dir)
          .then(function doCheckDir(stats) {
            if (stats.isFile() && /_exile/.test(dir)) {
              console.log(
                chalk.gray(dir),
                'seems to be a file on your filesystem! It needs to be either non-existent or a directory, so please rename your _exile post before using this command.'
              );
              process.exit();
              ...
              console.log(
                chalk.gray(dir),
                "seems to be neither a file nor a directory! This really shouldn't be happening, please issue a bug report."
              );
              process.exit();
            }
          })
      ...
      function move(origin, destination) {
        ...
        Promise.resolve(isDir(assetDir)).then(function checkAssetDir(exists) {
          if (exists) {
            fsRename(assetDir, assetDirDestination)
              .then(function doAssetDirMove() {
        ...
        return fsRename(origin, destination)
