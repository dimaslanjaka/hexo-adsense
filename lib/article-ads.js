 ...
 */
function filter_adshtml(options, hexo) {
  // parse article ads
  /**
   * @type {string[]}
   */
...
function after_post_render(data) {
  ...
  const hexo = this;
  const options = require("./config")(hexo);
  //utils.dump("after_post_render.txt", data.full_source);

  const filterPattern = filter_patterns(data.content, data.full_source, options);
   ...
   * @type {string[]}
   */
  let adshtml = filter_adshtml(options, hexo);
  if (adshtml.length < 1) {
    return data;
  }
...
function after_render_html(content, data) {
  ...
  const filterPattern = filter_patterns(content, page.full_source, hexo);
  if (filterPattern.excluded) {
