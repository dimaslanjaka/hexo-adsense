...
let isLocal = false;
checkLocalHost.default("127.0.0.1").then(function (yes) {
...
function injectAdsContent(content, data) {
  ...
  if (typeof toDebug.adsense == "boolean") {
    if (toDebug.adsense) {
      ...
      const regexEndBody = /<\/body\>/gm;
      content = content.replace(regexEndBody, `${replacement(adshtml)}</body>`);
      ...
      const i = utils.randInt(0, linebreaks.length);
      let linebreak = linebreaks.item(i);
      //console.log(typeof linebreak, linebreak.toString(), i);
      if (adshtml.length > 0) {
        //htmlp.insertAfter(htmlp.createNodeFromHtml(adshtml[0]), linebreak);
        insertAfter(linebreak, htmlp.createNodeFromHtml("<i>iam here</i>"));
        adshtml.shift();
      }
      */
    }
