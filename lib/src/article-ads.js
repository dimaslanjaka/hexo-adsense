...
var md5 = require("./md5");
var adsContent = "<div hexo-adsense=\"ads-content\">ADS_CODE_HERE</div>";
...
function articleAds(files, hexo) {
    ...
    var isDevelopment = require("./config")(hexo).development;
    ...
    files.forEach(function (file) {
         ...
         */
        var html;
        var tes = base_dir.joinpath(file);
...
function after_generate() {
     ...
     * @type {Hexo}
     */
    var hexo = this, route = hexo.route, options = require("./config")(hexo);
    //console.log(arguments.length);
    // Filter routes to select all html files.
    var routes = route.list().filter(function (path0) {
        var choose = minimatch(path0, "**/*.{htm,html}", { nocase: true });
...
function filter_adshtml(hexo) {
    ...
    if (adshtml.length > 0) {
        adshtml = adshtml.filter(function (ads) {
...
var filter_patterns = function (content, source_path, hexo) {
    ...
    if (source_path && excluded && !minimatch(source_path, "**/*.{htm,html}", { nocase: true })) {
        log.log((_a = {}, _a[source_path] = excluded, _a), "isnt html, skipping...");
        return {
            content: content,
            excluded: true
        };
...
function after_post_render(data) {
    ...
    var filterPattern = filter_patterns(data.content, getPageSource(data), hexo);
     ...
     * @type {string[]}
     */
    var adshtml = filter_adshtml(hexo);
    if (adshtml.length < 1) {
        return data;
    }
...
function processAdsHtml(content, adshtml, hexo) {
    var options = require("./config")(hexo);
    var replacement = memoize(function (adshtml) {
        var adsContentCss = __dirname.joinpath("../source/article-ads.css");
        var adsContentJs = __dirname.joinpath("../source/article-ads.min.js");
        var replacement = "<div id=\"hexo-adsense-hidden\" style=\"display:none\">".concat(adshtml.join(""), "</div>");
        if (fs.existsSync(adsContentCss)) {
            replacement += "<style>".concat(fs.readFileSync(adsContentCss).toString(), "</style>");
        }
        if (fs.existsSync(adsContentJs)) {
            replacement += "<script>".concat(fs.readFileSync(adsContentJs).toString(), "</script>");
        }
        return replacement;
    });
    var caller = new Error().stack.split("at ")[2].replace(/^Hexo./g, "");
    var isFieldPost = caller.startsWith("after_post_render");
    ...
    if (isFieldPost) {
        ...
        if (!options.enable) {
            content = content.replace(/<\/head\>/gm, "<script async src=\"//pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=".concat(options.pub, "\" crossorigin=\"anonymous\"></script>"));
        }
        content = content.replace(/<\/body\>/gm, "".concat(replacement(adshtml), "</body>"));
    }
    return content;
}
/**
...
function after_render_html(content, data) {
    ...
    delete page.excerpt;
    log.log(page);*/
    var filterPattern = filter_patterns(content, getPageSource(page), hexo);
    if (filterPattern.excluded) {
...
module.exports = {
    ...
    after_render_html: after_render_html,
